...
#SBATCH --mem=40G
#SBATCH -t 6-23:59:00
#SBATCH -o log_Annotsv.out
#SBATCH -e log_Annotsv.err
#SBATCH -J Annotsv
...
# Create VCF file to use it as input
...
awk -F '\t' '{if($1!~/^#/ && $14>'$nprograms'){print $5"\t"$6-1"\t"$6}}'  ${combined_file} |sed 1d   > ${combined_file_tmp} # here take into account the fai...
...
# Annotate 
apptainer exec PATH_TO_CONTAINERS_DIR/annotsv\:3.4.4--py312hdfd78af_0 AnnotSV/bin/AnnotSV \
-SVinputFile "$dir/${outputfile}.vcf" \
-bcftools PATH_TO_BCFTOOLS/bcftools \
-bedtools PATH_TO_BEDTOOLS/bedtools \
-genomeBuild GRCh38 \
-outputfile "$2"
